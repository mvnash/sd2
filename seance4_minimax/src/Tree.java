...
public class Tree {
  ...
  public Tree(State state) {
    this.state = state;
    if (state.getLeftBar() == state.getRightBar()) return;
  ...
  private static Triplet minBlue(Triplet leftRes, Triplet rightRes) {
    if (leftRes.getMinBlue() > rightRes.getMinBlue()) return new Triplet(true, leftRes.getMinBlue(), leftRes.getMinOrange());
  ...
  private static Triplet minOrange(Triplet leftRes, Triplet rightRes) {
    if (leftRes.getMinOrange() > rightRes.getMinOrange()) return new Triplet(true, leftRes.getMinBlue(), leftRes.getMinOrange());
    return new Triplet(false, rightRes.getMinBlue(), rightRes.getMinOrange());
  }
  ...
  public void computeMinimaxValues() {
    if (this.isLeaf()){
      minimaxValue = new Triplet(true,this.state.getBluePoints(),this.state.getOrangePoints());
    } else if (this.state.isBlueToPlay()){
